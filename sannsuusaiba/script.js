...
const weaponData = [
  ...
  { name: "サイバーブレード", req: {shizuku: 0, hone: 0, kouseki: 150, maseki: 100}, reqText: "(🔸x150, 🔮x100)" },
  { name: "プラズマキャノン", req: {shizuku: 0, hone: 0, kouseki: 200, maseki: 150}, reqText: "(🔸x200, 🔮x150)" }
];
...
const armorData = [
  ...
  { name: "ナノアーマー", hpBonus: 110, req: {shizuku: 0, hone: 0, kouseki: 150, maseki: 100}, reqText: "(🔸x150, 🔮x100)" },
  { name: "量子バリア", hpBonus: 150, req: {shizuku: 0, hone: 0, kouseki: 200, maseki: 150}, reqText: "(🔸x200, 🔮x150)" }
];
...
function useMagic(type) {
  if (gold < 50) {
    ...
    msgEl.innerText = "CRDT不足！(50CR 必要)";
    return;
  }
  gold -= 50; updateStatus();
  if (type === 'hint') {
    document.getElementById("battle-message").innerText = `HACKING SUCCESS: ANSWER = [ ${currentAnswer} ]`;
  }
}
...
function resetData() { if (confirm("[SYS_WARNING] 本当に 全データを消去して 初期化しますか？")) { localStorage.clear(); location.reload(); } }
...
function updateStatus() {
  ...
  document.getElementById("companions-list").innerHTML = companions.length > 0 ? companions.join("") : "NULL";
  ...
  if (weaponLevel < weaponData.length - 1) { btnWeapon.innerHTML = `ぶきを きたえる<br><span style="font-size:10px; color:var(--neon-yellow);">${weaponData[weaponLevel + 1].reqText}</span>`;
  } else { btnWeapon.innerHTML = `MAXIMUM_WEAPON`; }

  const btnArmor = document.getElementById("btn-craft-armor");
  if (armorLevel < armorData.length - 1) { btnArmor.innerHTML = `ぼうぐを きたえる<br><span style="font-size:10px; color:var(--neon-yellow);">${armorData[armorLevel + 1].reqText}</span>`;
  } else { btnArmor.innerHTML = `MAXIMUM_ARMOR`; }
...
function checkTelescope() {
  document.getElementById("shop-message").innerText = "> スキャン完了。未知のセクターがアンロックされるかもしれない。";
}
...
function checkLevelUp() {
  if (exp >= nextExp) {
    ...
    alert(`[SYSTEM] LEVEL UP! LV${level} に 到達。\n最大HPが 上昇、フル・リカバリー完了。`);
  }
}

function stayInn() {
  if (currentPlayerHp === playerMaxHp) { document.getElementById("shop-message").innerText = "> HPはすでにMAXだ。"; return; }
  if (gold >= 10) { gold -= 10; currentPlayerHp = playerMaxHp; document.getElementById("shop-message").innerText = `> リカバリー完了。HPが全快した。`; updateStatus();
  } else { document.getElementById("shop-message").innerText = `> ERROR: CRDTが不足している。`; }
}

function buyPotion() {
  if (gold >= 30) { gold -= 30; potions++; document.getElementById("shop-message").innerText = `> 「やくそう(RECOVER)」をダウンロードした！`; updateStatus();
  } else { document.getElementById("shop-message").innerText = `> ERROR: CRDTが不足している。`; }
}
...
function usePotion() {
  if (potions <= 0) {
    let msgEl = document.getElementById("battle-area").style.display === "block" ? document.getElementById("battle-message") : document.getElementById("map-message");
    msgEl.innerText = "> ERROR: RECOVER アイテムがない！";
    return;
  }
  if (currentPlayerHp >= playerMaxHp) {
    let msgEl = document.getElementById("battle-area").style.display === "block" ? document.getElementById("battle-message") : document.getElementById("map-message");
    msgEl.innerText = "> HPは すでにMAXだ。";
    return;
  }
  potions--;
  currentPlayerHp += 15;
  ...
  if (document.getElementById("battle-area").style.display === "block") {
    document.getElementById("battle-message").innerText = "> RECOVER 使用！ HP回復完了！";
  } else {
    document.getElementById("map-message").innerText = "> RECOVER 使用！ HP回復！";
  }
}

function craftArmor() {
  if (armorLevel >= armorData.length - 1) { document.getElementById("shop-message").innerText = `> 現在の技術では これ以上 強化できない。`; return; }
  ...
  if (materials.shizuku >= req.shizuku && materials.hone >= req.hone && materials.kouseki >= req.kouseki && materials.maseki >= req.maseki) {
    ...
    document.getElementById("shop-message").innerText = `> UPGRADE SUCCESS: [ ${armorData[armorLevel].name} ] を インストールした！`; updateStatus();
  } else { document.getElementById("shop-message").innerText = `> ERROR: 素材データが 不足している。`; }
}

function craftWeapon() {
  if (weaponLevel >= weaponData.length - 1) { document.getElementById("shop-message").innerText = `> 現在の技術では これ以上 強化できない。`; return; }
  const req = weaponData[weaponLevel + 1].req;
  if (materials.shizuku >= req.shizuku && materials.hone >= req.hone && materials.kouseki >= req.kouseki && materials.maseki >= req.maseki) {
    materials.shizuku -= req.shizuku; materials.hone -= req.hone; materials.kouseki -= req.kouseki; materials.maseki -= req.maseki; 
    weaponLevel++; document.getElementById("shop-message").innerText = `> UPGRADE SUCCESS: [ ${weaponData[weaponLevel].name} ] を インストールした！`; updateStatus();
  } else { document.getElementById("shop-message").innerText = `> ERROR: 素材データが 不足している。`; }
}

function playGacha() {
  if (gold < 100) { document.getElementById("shop-message").innerText = `> ERROR: ガチャには 100CR 必要だ。`; return; }
  gold -= 100; updateStatus();
  document.getElementById("shop-message").innerText = `> SCANNING...`;
  ...
  setTimeout(() => {
    ...
    if (r < 0.15) {
      ...
      if (!companions.includes(rare)) {
        companions.push(rare); document.getElementById("shop-message").innerText = `> S-RARE HIT!! [ ${rareName} ] と リンクが完了した！`; 
      } else {
        gold += 100; document.getElementById("shop-message").innerText = `> S-RARE HIT!! だが 既にリンク済だ。100CR 返却する。`; 
      }
    } else if (r < 0.35) {
      materials.kouseki += 1; materials.maseki += 1; document.getElementById("shop-message").innerText = "> DROP: RARE MATERIAL [ 🔸x1, 🔮x1 ]";
    } else {
      materials.shizuku += 1; materials.hone += 1; document.getElementById("shop-message").innerText = "> DROP: MATERIAL [ 💧x1, 🦴x1 ]";
    }
    updateStatus();
  }, 300);
}
...
function tryStartGame(type, min, max, reqLv) {
  if (weaponLevel < reqLv) { 
    document.getElementById("shop-message").innerText = `> ACCESS DENIED: セキュリティLV [${weaponData[reqLv].name}] 以上が必要。`; 
    return; 
  }
...
function generateMap() {
  document.getElementById("dungeon-info").innerText = ">> EXPLORING_SECTOR <<";
  document.getElementById("map-message").innerText = "> TARGET: エリア内のボスを索敵せよ"; 
  ...
  let bHp = 20; let bRwd = 50; let bXp = 30; let bIcon = '👿'; let bName = "ウイルス"; let bAtk = 3;
  if (stageType === "kuku") { bHp = 15; bIcon = '👾'; bAtk = 2; }
  else if (stageType === "kuku_boss") { bHp = 40; bRwd = 100; bXp = 50; bAtk = 5; }
  else if (stageType === "kuku_all") { bHp = 60; bRwd = 200; bXp = 100; bIcon = '💀'; bName = "マスター・トロイ"; bAtk = 8; }
  else if (stageType === "kuku_puzzle") { bHp = 35; bRwd = 80; bXp = 40; bIcon = '🧩'; bName = "暗号キューブ"; bAtk = 5; }
  else if (stageType.startsWith("hissan")) { bHp = 80; bRwd = 400; bXp = 200; bIcon = '🐉'; bName = "サイバー・ドラゴン"; bAtk = 12; }
  else if (stageType === "weight") { bHp = 30; bIcon = '🤖'; bName = "メカゴーレム"; bAtk = 6; }
  else if (stageType === "decimal" || stageType === "dec_multdiv") { bHp = 50; bIcon = '📡'; bName = "AIコマンダー"; bAtk = 8; }
  else if (stageType === "fraction") { bHp = 60; bIcon = '⚙️'; bName = "ギアマスター"; bAtk = 10; }
  else if (stageType === "add3x3_carry1") { bHp = 45; bIcon = '🔥'; bName = "ファイヤー・ウォール"; bAtk = 7; } 
  else if (stageType === "add3x3_carry10") { bHp = 50; bIcon = '💥'; bName = "オーバーロード"; bAtk = 8; } 
  else if (stageType === "add3x3_carry2") { bHp = 60; bIcon = '☄️'; bName = "サテライト・キャノン"; bAtk = 9; } 
  else if (stageType === "add3x3_over1000") { bHp = 70; bIcon = '🐺'; bName = "サイバーウルフ"; bAtk = 10; } 
  else if (stageType === "add_large") { bHp = 80; bIcon = '🦖'; bName = "ギガント・メカ"; bAtk = 12; } 
  else if (stageType === "sub3x3_borrow1") { bHp = 45; bIcon = '🦇'; bName = "ステルスバット"; bAtk = 7; }
  else if (stageType === "sub3x3_borrow2") { bHp = 55; bIcon = '🧛'; bName = "バンパイア・ハッカー"; bAtk = 9; }
  else if (stageType === "sub_large") { bHp = 80; bIcon = '🦅'; bName = "ブラックドローン"; bAtk = 12; }
  else if (stageType.startsWith("add") || stageType.startsWith("sub")) { bHp = 50; bIcon = '💠'; bName = "メインフレーム"; bAtk = 8; }
  else if (stageType === "div_rem") { bHp = 60; bIcon = '➗'; bName = "ディバイド・システム"; bAtk = 10; }
  else if (stageType === "area_calc") { bHp = 90; bIcon = '📐'; bName = "ジオ・アーキテクト"; bAtk = 15; }
  else if (stageType === "time_seconds") { bHp = 70; bIcon = '⏳'; bName = "タイムスリップAI"; bAtk = 10; }
  else if (stageType === "sun_shadow") { bHp = 70; bIcon = '🌞'; bName = "ソーラーコア"; bAtk = 10; }
  else if (stageType === "time_calc") { bHp = 80; bIcon = '🕰️'; bName = "クロックマスター"; bAtk = 12; }
  else if (stageType === "time_calc_minutes") { bHp = 80; bIcon = '⏳'; bName = "クロックジェネレータ"; bAtk = 12; }
  else if (stageType === "time_conversion") { bHp = 80; bIcon = '🌠'; bName = "メテオストライク"; bAtk = 12; }
  else if (stageType === "time_conversion2") { bHp = 85; bIcon = '🌀'; bName = "ワームホール"; bAtk = 12; }
  else if (stageType === "mixed_hard") { bHp = 200; bRwd = 1500; bXp = 800; bIcon = '🌐'; bName = "マザー・システム"; bAtk = 25; }
  else if (stageType === "missing_multdiv") { bHp = 100; bRwd = 500; bXp = 300; bIcon = '👁️'; bName = "ビッグ・ブラザー"; bAtk = 20; }
  else if (stageType === "large_mental") { bHp = 120; bRwd = 600; bXp = 350; bIcon = '🧠'; bName = "ニューラル・ブレイン"; bAtk = 22; }
  ...
  for (let dy = -viewRadius; dy <= viewRadius; dy++) {
    for (let dx = -viewRadius; dx <= viewRadius; dx++) {
      ...
      if (vx < 0 || vx >= largeMapSize || vy < 0 || vy >= largeMapSize) {
        ...
        else if (vx === bossX && vy === bossY) { cell.innerHTML = `<span class="map-entity" style="filter: drop-shadow(0 0 10px red);">${currentBoss.icon}</span>`; } 
        else if (isChest) { cell.innerHTML = `<span class="map-entity">💾</span>`; } 
        else if (isSpring) { cell.innerHTML = `<span class="map-entity" style="color:var(--neon-green); text-shadow: 0 0 10px var(--neon-green);">🔋</span>`; } 
  ...
  if (newX >= 0 && newX < largeMapSize && newY >= 0 && newY < largeMapSize) {
    ...
    if (chestIndex !== -1) {
      ...
      if (Math.random() < 0.20) {
        ...
        gold += bonusGold; document.getElementById("map-message").innerText = `💾 DATA_CACHE! ${bonusGold}CR ゲット！`;
        updateStatus();
      }
    }
    ...
    if (springIndex !== -1) {
      ...
      document.getElementById("map-message").innerText = `🔋 CHARGING_STATION! HPが 大きく かいふく した！`;
    ...
    if (fItemIndex !== -1) {
      ...
      document.getElementById("map-message").innerText = `🖐️ 【${matNames[item.type]}】を 回収した！`;
...
function encounterTrap() {
  ...
  document.getElementById("battle-message").innerText = "[SECURITY ALERT] ワナが作動！\n急いで けいさん しろ！"; 
  generateQuestion();
}
...
function encounterMimic() {
  ...
  document.getElementById("battle-area").style.display = "block"; document.getElementById("clear-area").style.display = "none";
  document.getElementById("monster-img").innerHTML = "📦"; 
  document.getElementById("battle-message").innerText = "[TRAP] データキャッシュは偽装だった！\nウイルスが おそいかかってきた！"; 
  generateQuestion();
}
...
function encounterBoss() {
  ...
  document.getElementById("battle-message").innerText = `[TARGET ACQUIRED] ボスの ${currentBoss.name} が たち はだかった！`;
  generateQuestion();
}
...
function encounterVisibleMonster(index) {
  ...
  document.getElementById("battle-message").innerText = `[ALERT] ${enemy.name} が あらわれた！`;
  generateQuestion();
}
...
function encounterRandomMonster() {
  ...
  document.getElementById("battle-message").innerText = `[ALERT] とつぜん ${baseEnemy.name} が あらわれた！`;
  generateQuestion();
}
...
function playAttackEffect(wLv, isCrit) {
  ...
  // 進化武器エフェクト対応
  if (wLv >= 10) { effect.innerHTML = `<div class="effect-slash" style="transform: rotate(45deg); box-shadow: 0 0 25px #00f3ff, 0 0 50px #00f3ff;"></div><div class="effect-slash" style="transform: rotate(-45deg); animation-delay: 0.1s;"></div><div class="effect-slash" style="transform: rotate(90deg); animation-delay: 0.2s;"></div><div class="effect-boom" style="animation-delay: 0.25s;">🌐💥</div>`; }
  else if (wLv >= 8) { effect.innerHTML = `<div class="effect-slash" style="transform: rotate(45deg); box-shadow: 0 0 25px #e91e63, 0 0 50px #e91e63;"></div><div class="effect-slash" style="transform: rotate(-45deg); animation-delay: 0.1s;"></div><div class="effect-slash" style="transform: rotate(90deg); animation-delay: 0.2s;"></div><div class="effect-boom" style="animation-delay: 0.25s;">🌌💥</div>`; }
...
function checkAnswer() {
  ...
  if (document.getElementById("math-vertical-addsub").style.display === "block") {
    ...
    if (ans === "") return;
    if (parseInt(ans) === currentAnswer) isCorrect = true; else failMessage = `> ERROR: 計算エラー！`;
    ...
    if (parseInt(ans) === currentAnswer && parseInt(rAns) === currentRem) {
      ...
      if (parseInt(ans) !== currentAnswer) {
          failMessage = isTime ? `> ERROR: 「分」が違う！` : `> ERROR: 商が違う！`;
      } else {
          failMessage = isTime ? `> ERROR: 「秒」が違う！` : `> ERROR: 余りが違う！`;
      }
    }
    ...
    if (ans === "") return;
    if (Math.abs(parseFloat(ans) - currentAnswer) < 0.01) isCorrect = true; else failMessage = `> ERROR: 不正解！`;
    
  } else if (document.getElementById("v-step2-area").style.display === "none") {
    const ans = document.getElementById("v-step1").value;
    if (ans === "") return;
    if (parseInt(ans) === currentNum1 * currentNum2) isCorrect = true; else failMessage = `> ERROR: 計算エラー！`;
  } else {
    ...
    if (parseInt(s1) !== trueS1) failMessage = `> ERROR: STEP1の計算エラー！`;
    else if (parseInt(s2) !== trueS2) failMessage = `> ERROR: STEP2の計算エラー！`;
    else if (parseInt(ans) !== trueAns) failMessage = `> ERROR: 最終合算エラー！`;
    else isCorrect = true;
  }
  ...
  if (isCorrect) {
    ...
    if (isTrapEncounter) {
      document.getElementById("monster-img").innerText = "💨"; document.getElementById("battle-message").innerText = "> EVADE: トラップの回避に成功！";
    ...
    if (currentMonsterHp > 0) {
      if (isCrit) { document.getElementById("battle-message").innerText = `> CRITICAL HIT!${assistMsg}\n(ENEMY HP: ${currentMonsterHp})`;
      } else { document.getElementById("battle-message").innerText = `> SUCCESS!${assistMsg}\n(ENEMY HP: ${currentMonsterHp})`; }
      generateQuestion();
    } else {
      if (isBossBattle || isMimic) {
        ...
        gold += rwd; exp += xpb; 
        document.getElementById("monster-img").innerHTML = "💥"; 
        document.getElementById("battle-message").innerText = `> ${isMimic ? "ウイルス" : "ターゲット"}を 撃破した！${assistMsg}\n${xpb}EXP, ${rwd}CR 取得！`;
        ...
        let dropMessage = "";
        if (monster.drop && Math.random() < 0.6) { const matKey = monster.drop; materials[matKey]++; dropMessage = `\n> DATA [${matNames[matKey]}] 取得！`; }
        let joinMessage = "";
        if (Math.random() < 0.2 && !companions.includes(monster.icon) && monster.name !== "マスター・トロイ") { companions.push(monster.icon); joinMessage = `\n> ${monster.name} と リンクを確立した！`; }

        document.getElementById("battle-message").innerText = `> DESTROYED!${assistMsg}\n${monster.expBonus}EXP, ${monster.reward}CR!${dropMessage}${joinMessage}`;
        checkLevelUp(); updateStatus(); document.getElementById("battle-area").style.display = "none";
...
function gameOver() {
  ...
  document.getElementById("battle-message").innerText = `> SYSTEM SHUTDOWN...\n接続が切れました。`;
  document.getElementById("battle-area").style.display = "none"; document.getElementById("clear-area").style.display = "block";
}

function stageClear() {
  stopTimer();
  if (isHardMode) { document.getElementById("monster-img").innerHTML = "👑"; document.getElementById("battle-message").innerText = `> MISSION COMPLETE!!\nファイアウォール突破に成功した！`;
  } else { document.getElementById("monster-img").innerHTML = "💾"; gold += 50; updateStatus(); document.getElementById("battle-message").innerText = `> MISSION CLEAR!\nボーナス 50CR 取得！`; }
  document.getElementById("battle-area").style.display = "none"; document.getElementById("clear-area").style.display = "block";
}

function escapeDungeon() { if (confirm("現在のセッションを破棄してシステムからログアウトしますか？")) { goToTown(); } }
...
function importData() {
  ...
  if (str.length !== 8) {
    alert("[ERROR] 暗号は ひらがな8文字だ。確認してくれ！");
    return;
  }
  ...
  for (let i = 7; i >= 0; i--) {
    ...
    if (idx === -1) {
      alert("[ERROR] 読み取れない文字がある。確認してくれ！");
      return;
    }
    n = n * 64 + idx;
  }
